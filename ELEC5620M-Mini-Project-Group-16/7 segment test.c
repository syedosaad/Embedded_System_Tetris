...
#define KEY_PTR           ((volatile uint32_t *) 0xFF200050)
...
// Delay loop for debounce (adjust as needed)
void delay(volatile int count) {
    while (count-- > 0);
}

void display_number(int num) {
    uint32_t value_3_0 = 0;
    uint32_t value_5_4 = 0;
...
int main(void) {
    ...
    while (1) {
        if ((*KEY_PTR & 0x1) == 0) {
            // Wait for KEY0 press (bit 0 == 0 when pressed)
            continue;
        }

        delay(100000); // debounce delay
        while ((*KEY_PTR & 0x1) != 0); // wait for release

        score++;
        if (score > 999999) score = 0; // reset after 6 digits
        display_number(score);
    }
